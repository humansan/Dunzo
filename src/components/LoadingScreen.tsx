...
export const LoadingScreen: React.FC<{ message: string }> = ({ message }) => (
  <div className="h-screen flex flex-col items-center justify-center gap-5 bg-canvas text-fg-faint text-sm">
    <img src={appLogo} alt="" className="w-16 h-16" />
    <LoaderCircle className="w-6 h-6 animate-spin [animation-duration:2s] text-fg-subtle" />
    <p>{message}</p>
  </div>
);
