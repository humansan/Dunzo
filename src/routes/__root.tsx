...
function RootErrorComponent({ error }: ErrorComponentProps) {
  return (
    <div className="h-screen flex flex-col items-center justify-center gap-4 bg-canvas text-fg/60 text-sm text-center px-6">
      <p className="text-fg/80 font-semibold">Something went wrong.</p>
      {error?.message && (
        <p className="text-fg/40 max-w-md break-words">{error.message}</p>
      )}
      <button
