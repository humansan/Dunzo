    ...
    >
      {titleChrome ? (
        ...
        <>
          ...
          <div
            className="grid sticky top-0 z-30 w-max min-w-full bg-canvas border-y border-line h-9"
          ...
          >
            {effectiveColumns.map((c, idx) => (
              <div
                ...
                className={`${headerCellCls} ${idx > 0 ? 'border-l border-line-subtle' : ''} ${
                  idx === 0 ? 'sticky left-0 z-10 bg-canvas border-r border-line-subtle' : ''
                } ${idx === effectiveColumns.length - 1 ? 'border-r border-line-subtle' : ''}`}
              >
